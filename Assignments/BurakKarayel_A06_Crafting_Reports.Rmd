...
```{r project.setup, message=FALSE, warning=FALSE, error=FALSE, results='hide', echo=TRUE}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
knitr::kable(summary_PM25, col.names = c("County", "$\\mu g/m^3$"), caption = "Mean Particulates (2.5mm)", digits = 2)

knitr::kable(summary_Ozone, col.names = c("County", "$\\mu g/m^3$"), caption = "Mean Ozone", digits = 2)
```
...
```{r echo=FALSE, fig.cap=" 2018 monthly Ozone levels", fig.height=4}
#Take only 2018 data with Month and ozone data to a new dataset.
Ozone_18 <- epa_data %>%
  filter(Year =="2018") %>%
  select(Month, Ozone)

#Transform Month column to a new column which consists of month names.
Ozone_18 <- Ozone_18 %>%
 mutate(Jan2Dec = factor(month.abb[Month],levels=month.abb))

#Plot months vs. ozone levels. 
ggplot(Ozone_18, aes(x=Jan2Dec, y=Ozone)) +
  geom_boxplot(na.rm=TRUE) + #drop NA values from the plot not to get error
 labs(title="Monthly Ozone concentrations (2018)",
 x="Month",
 y="Concentration (µg/m³)") #can't figure out how to use "$\\mu g/m^3$". It didn't work. 
```


```{r echo=FALSE, fig.cap=" 2019 monthly Ozone levels", fig.height=4}
#Take only 2018 data with Month and ozone data to a new dataset.
Ozone_19 <- epa_data %>%
  filter(Year =="2019") %>%
  select(Month, Ozone)
...
#Plot months vs. ozone levels. 
ggplot(Ozone_19, aes(x=Jan2Dec, y=Ozone)) +
  geom_boxplot(na.rm=TRUE) + #drop NA values from the plot not to get error
...
## Git Repository
[My GitHub Repository](https://github.com/kbkarayel/EDA_Spring2025_kbk)
